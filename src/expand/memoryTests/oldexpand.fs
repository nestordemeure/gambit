...
and Node<'State,'Context> = { context:'Context; children:Tree<'State,'Context> array
                              state:'State Option; stack:'State list; mutable age:int}
...
/// returns the age of the oldest children or, by default, the current number of evaluations
let ageOfChildren (result:Result<_,_>) children =
   let mutable result = result.nbEvaluation
   for child in children do 
      match child with 
      | Node node when node.age < result -> result <- node.age
      | _ -> ()
   result 

/// returns the index of the oldest children
let indexOldest children =
   let mutable bestAge = System.Int32.MaxValue
   let mutable bestChild = 0
   for i = 0 to Array.length children - 1 do 
      match children.[i] with 
      | Node node when node.age < bestAge -> 
         bestAge <- node.age 
         bestChild <- i
      | _ -> ()
   bestChild
...
let rec expand depth grammar (algorithm:Algorithm<_>) (result:Result<_,_>) nbAvailableNodes formula tree =
   ...
   | Node node -> // a node
      ...
      | eval, children -> // updated childrens, we rebuild the node
         let node = Node {node with context=algorithm.update node.context eval; children=children; age=ageOfChildren result children}
         eval, Some node
   | Contracted (state::stack as oldStack, context) -> // a previously contracted node
      ...
      | [||] -> // terminal state 
         ...
         let node = Node {context=context; children=children; state=Some state; stack=stack; age=result.nbEvaluation}
      ...
      | children -> // non terminal state
         ...
         let node = Node {context=context; children=children; state=None; stack=oldStack; age=result.nbEvaluation}
         decr nbAvailableNodes
   ...
   | Leaf (state::stack as oldStack) -> // an unexpanded leaf
      ...
      | [||] -> // terminal state 
         ...
         let node = Node {context=algorithm.defaultContext; children=children; state=Some state; stack=stack; age=result.nbEvaluation}
         decr nbAvailableNodes
         let eval, treeOpt = expand depth grammar algorithm result nbAvailableNodes formula node
         eval, treeOpt
      | children -> // non terminal state
         let children = Array.map (fun rule -> Leaf (rule@stack)) children
         let node = Node {context=algorithm.defaultContext; children=children; state=None; stack=oldStack; age=result.nbEvaluation}
         decr nbAvailableNodes
         let eval, treeOpt = expand (depth-1) grammar algorithm result nbAvailableNodes formula node
         eval, treeOpt
...
let rec contract algorithm result nbAvailableNodes tree =
   match tree with 
   | Leaf _ | Contracted _ -> false
   | Node node ->
      let indexChild = indexOldest node.children
      let child = node.children.[indexChild]
      match child with 
      | Leaf _ | Contracted _ -> false
      | Node _ when contract algorithm result nbAvailableNodes child -> 
         node.age <- ageOfChildren result node.children
         true 
      | Node childNode ->
...
let search result expand algorithm grammar maxIterations memorysize =
   ...
   while result.nbEvaluation < maxIterations do 
      if !nbAvailableNodes <= 0 then 
         contract algorithm result nbAvailableNodes tree |> ignore
      else 
