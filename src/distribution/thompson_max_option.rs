...
impl Distribution for ThompsonMaxOption
{
   type ScoreType = Option<f64>;
   
   /// returns a default, empty, prior
   fn new() -> ThompsonMaxOption
   {
      ThompsonMaxOption { nb_visit: 0, nb_score: 0, sum_scores: 0., max_score: std::f64::NEG_INFINITY }
   }
   ...
   fn score<RNG: Rng>(&self, default_distribution: &ThompsonMaxOption, mut rng: &mut RNG) -> f64
   {
      if self.nb_visit == 0
      {
